...
---
## This repo: 513sips-books-deploy

- `Egady513/513sips-books-deploy`
- React + TypeScript + Vite + Supabase

**Critical build rule: run `npm run build` before EVERY commit.** Not
`tsc --noEmit`. CI enforces `noUnusedLocals`, so a build failure is a broken
deploy. This is the single most common way this repo breaks.

- Accounting is accrual-basis (locked 2026-04-24). Expense plus AP are two sides
  of one transaction, not double counting.
- `513sips-tools/books/` is a stale copy. Ignore it. This repo only.
